...
const ENDPOINTS = {
  GET_VIDEO_COMMENTS: `https://www.googleapis.com/youtube/v3/commentThreads`
}
...
const fetchVideoComments = (props:fetchVideoCommentsProps) => {
  ...
  const query = createQuery<GetVideoCommentsResponse | any>(
    () => ['comments'],
    async () => {
      return fetch(`${ENDPOINTS.GET_VIDEO_COMMENTS}?key=${request.key}&part=${request.part}&videoId=${request.videoId}`)
        .then(res => {
